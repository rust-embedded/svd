...
use svd_parser::svd::{
    self, clusterinfo::ClusterInfoBuilder, enumeratedvalues::EnumeratedValuesBuilder,
    fieldinfo::FieldInfoBuilder, peripheral::PeripheralBuilder, registerinfo::RegisterInfoBuilder,
    Access, AddressBlock, AddressBlockUsage, BitRange, Cluster, ClusterInfo, Cpu, Device,
    DimElement, Endian, EnumeratedValue, EnumeratedValues, Field, FieldInfo, Interrupt, Peripheral,
    Register, RegisterCluster, RegisterInfo, RegisterProperties, Usage, ValidateLevel,
...
impl GetVal for Hash {
    ...
    fn hash_iter<'a, K: ToYaml>(&'a self, k: K) -> HashIter<'a> {
        OptIter(self.get_hash(k).map(|h| h.iter()))
    }
    fn get_vec<K: ToYaml>(&self, k: K) -> Option<&Vec<Yaml>> {
        self.get(&k.to_yaml()).and_then(Yaml::as_vec)
    }
    fn str_vec_iter<'a, K: ToYaml>(&'a self, k: K) -> OptIter<&'a str, OverStringIter<'a>> {
        OptIter(self.get(&k.to_yaml()).map(|y| OverStringIter(y, None)))
    }
}
...
type FieldIterMut<'a> = OptIter<&'a mut Field, std::slice::IterMut<'a, Field>>;
...
impl RegisterExt for Register {
    ...
    fn iter_all_fields<'a>(&'a mut self) -> FieldIterMut<'a> {
        OptIter(self.fields.as_mut().map(|f| f.iter_mut()))
    }
